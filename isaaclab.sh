...
install_isaaclab_extension() {
    ...
    if [ -f "$1/setup.py" ]; then
        echo -e "\t module: $1"
        ${python_exe} -m uv pip install --editable $1
    fi
}
...
while [[ $# -gt 0 ]]; do
    ...
    case "$1" in
        -i|--install)
            ...
            ${python_exe} -m uv pip install -e ${ISAACLAB_PATH}/source/isaaclab_rl["${framework_name}"]
            ${python_exe} -m uv pip install -e ${ISAACLAB_PATH}/source/isaaclab_mimic["${framework_name}"]
        ...
        -n|--new)
            ...
            ${python_exe} -m uv pip install -q -r ${ISAACLAB_PATH}/tools/template/requirements.txt
        ...
        -d|--docs)
            ...
            ${python_exe} -m uv pip install -r requirements.txt > /dev/null
            # build the documentation
